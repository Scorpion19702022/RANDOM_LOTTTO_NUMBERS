...
const LottoRandomNumbers: React.FC<InitialProps> = ({ isRandomNumbers, isPopup, randomNumbers, clearAllNumbers }) => {
	return (
		<section className='relative w-[90%] h-[35%] p-[2em] flex flex-col justify-around items-center md:h-[50%] md:justify-between md:pb-24'>
			<LottoPopupError isEmptyNumber={isPopup} />
			...
			<div className='w-full h-[20%] flex justify-between items-center md:w-[50%] md:h-[11%]'>
				<button
					...
					className='w-[40%] h-full border-[0.04em] border-gray-200 text-gray-200 cursor-pointer transition duration-300 md:w-[40%] lg:w-[45%] md:hover:bg-gray-800'
				>
			...
			<div className='w-full h-[50%] pt-4 flex justify-center items-center'>
				{isRandomNumbers.map((item, index) => {
					return (
						<span key={index} className=' h-full text-[1.6em] text-cyan-300 font-semibold md:text-[3em] lg:text-[4em]'>
